...
class PipelineConfig:
    ...
    MODELS = {
        "ts": {"path": "ts.pth", "params": {"win": 60, "thresh": 1.5}, "method": "ts"},
        "ica": {"path": "ica.pth", "params": {"comps": 4}, "method": "ica"},
        "combo": {
            "path": "combo.pth",
            "params": {"win": 60, "thresh": 1.8, "comps": 3},
            "method": "combo",
        },
    }
...
class SignalCleaner:
    ...
    def apply_ica(data: np.ndarray, comps: int = 4) -> np.ndarray:
        ...
        for segment in data:
            try:
                ...
                if np.linalg.matrix_rank(X) < safe_comps or np.any(
                    np.std(X, axis=0) < 1e-6
                ):
                    cleaned.append(segment)
                    continue
                ica = FastICA(
                    n_components=safe_comps, random_state=42, whiten="unit-variance"
                )
                S = ica.fit_transform(X)
                scores = [
                    abs(np.corrcoef(S[:, i], X[:, 0])[0, 1]) for i in range(safe_comps)
                ]
    ...
    def clean(cls, data: np.ndarray, method: str, params: dict) -> np.ndarray:
        if method == "ts":
            return cls.apply_ts(data, **params)
        elif method == "ica":
            return cls.apply_ica(data, **params)
        elif method == "combo":
...
def complete_wavelet_filter(signal, wavelet="sym10", level=6):
    ...
    for i in range(bandpassed.shape[1]):
        ...
        # Fix length mismatch
        if len(reconstructed) > len(bandpassed):
            reconstructed = reconstructed[: len(bandpassed)]
        elif len(reconstructed) < len(bandpassed):
            reconstructed = np.pad(
                reconstructed, (0, len(bandpassed) - len(reconstructed)), mode="edge"
            )
...
def load_models():
    ...
    for name, conf in PipelineConfig.MODELS.items():
        ...
        model.load_state_dict(
            torch.load(conf["path"], map_location=torch.device("cpu"))
        )
        model.eval()
        models[name] = model
...
if missing:
    ...
    st.info(
        "Please download them from your notebook output and place them in the same folder as this app."
    )
else:
        ...
        if not all(col in df.columns for col in required_cols):
            ...
            for ch in range(4):
                ...
                if np.std(ch_data) > 1e-6:
                    processed_for_model[:, ch] = (ch_data - np.mean(ch_data)) / np.std(
                        ch_data
                    )
            ...
            for i, (name, model) in enumerate(models.items()):
                ...
                X_cleaned = SignalCleaner.clean(
                    X_segments, conf["method"], conf["params"]
                )
            ...
            peaks, _ = find_peaks(
                smoothed, height=0.4, distance=int(PipelineConfig.FS * 0.35)
            )
            ...
            st.download_button(
                "Download Peak Locations CSV", csv, "qrs_peaks.csv", "text/csv"
            )
